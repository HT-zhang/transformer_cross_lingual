...
def read_instances_from_file(inst_file, max_sent_len, keep_case):
    ...
    print("It's reading {} file".format(str(inst_file))) 
        ...
        if 'tgt' in inst_file:
            for sent in tqdm(f):
                ...
                if sent_tuple[0] == 'zh':
                    ...
                    words.insert(0,'<ja>')
                if len(words)+1 > max_sent_len:
                    trimmed_sent_count += 1
                #because of inserting langid
                word_inst = words[:max_sent_len+1]
...
#def shuffle(word_insts):
#    '''shuffle the dataset'''
#    word_insts = np.array(word_insts)
#    word_insts = np.random.shuffle(word_insts)
#    #return word_insts
...
def build_vocab_idx(word_insts, min_word_count):
    ...
    word2idx = {
        ...
        Constants.ZH_WORD: Constants.ZH,
        Constants.JA_WORD: Constants.JA}
...
def main():
    ...
    parser.add_argument('-max_len', '--max_word_seq_len', type=int, default=80)
    ...
    opt.max_token_seq_len = opt.max_word_seq_len + 3 # include the <s> and </s>
    ...
    #shuffle the data 
    #shuffle(train_src_word_insts)
    #shuffle(train_tgt_word_insts)
    np.random.seed(13)
    np.random.shuffle(train_src_word_insts)
    np.random.seed(13)
    np.random.shuffle(train_tgt_word_insts)
